...
# === FEM / REM trong Python (linearmodels) ===
df = df.set_index(['id', 'year'])
y = df['y']
X = sm.add_constant(df[['x1', 'x2']])

# Fixed Effects
fe = PanelOLS(y, X, entity_effects=True).fit(cov_type='clustered',
                                              cluster_entity=True)
print(fe.summary)
...
# So sánh FE vs RE (xem hệ số và dùng Hausman test)
```
