...
**Không dùng ARDL** khi có biến I(2), hoặc khi dữ liệu là dữ liệu bảng nhiều đơn vị (cân nhắc [FEM/REM](/ecolab/model/fem-rem) hoặc panel ARDL/PMG).
...
* --- Ước lượng ARDL (chọn trễ tự động theo AIC) ---
...
ardl_model <- ardl(growth ~ fdi + open + inf,
                   data = df, order = c(2, 2, 2, 2))
summary(ardl_model)

# --- Kiểm định đường bao (Bounds F-test) ---
...
# --- Bounds test (thủ công: F-test trên hệ số mức) ---
# Xem thêm tài liệu statsmodels hoặc ardl-bounds package
```
